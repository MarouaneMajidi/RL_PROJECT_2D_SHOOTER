"""
DDA Agent Implementation

PPO-based agent for Dynamic Difficulty Adjustment.
"""
...
from .config import DDAConfig


class DDAAgent:
    """
    DDA Agent using PPO algorithm.
    
    Learns to adjust game difficulty to maintain balanced challenge.
    """
    ...
    def __init__(self, config: DDAConfig):
        ...
        Args:
            config: DDAConfig object with hyperparameters
        """
        self.config = config
        
        # Set device
        self.device = torch.device(config.device if torch.cuda.is_available() else "cpu")
        print(f"DDA Agent using device: {self.device}")
        
        # Initialize actor-critic network
        self.policy = DDAActorCritic(
    ...
    def select_action(self, state: np.ndarray, deterministic: bool = False) -> Tuple[int, float, float]:
        ...
        Args:
            state: Current DDA state observation
    ...
    def save(self, path: str):
        ...
        print(f"DDA Model saved to {path}")
    ...
    def load(self, path: str):
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(f"DDA Model file not found: {path}")
        ...
        print(f"DDA Model loaded from {path}")
