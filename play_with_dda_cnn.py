"""
Play the game with CNN-based DDA agent adjusting difficulty.

You control the player manually while the CNN DDA agent dynamically adjusts
game difficulty based on screenshots of the game.
"""
...
from agents.dda_agent import DDAStateExtractor, DifficultyManager
...
pygame.display.set_caption("Zombie Shooter - CNN DDA Mode (You Play!)")
...
def main(dda_model_path: str = "checkpoints_cnn/dda_cnn/best_model.pth"):
    """Run game with CNN-based DDA."""
    
    # Load CNN DDA agent
    print("Loading CNN DDA agent...")
    dda_config = CNNDDAConfig()
    dda_agent = CNNDDAAgent(dda_config)
    
    try:
        dda_agent.load(dda_model_path)
        print("CNN DDA agent loaded!")
        use_dda = True
    except FileNotFoundError:
        print(f"Warning: CNN DDA model not found at {dda_model_path}")
        print("Running without DDA (base difficulty)")
        use_dda = False
    except Exception as e:
        print(f"Error loading CNN DDA agent: {e}")
        print("Running without DDA (base difficulty)")
        use_dda = False
    ...
    # Initialize image preprocessor for CNN DDA
    image_preprocessor = ImagePreprocessor()
    image_preprocessor.ARENA_X_OFFSET = ARENA_X_OFFSET
    image_preprocessor.ARENA_Y_OFFSET = ARENA_Y_OFFSET
    image_preprocessor.ARENA_WIDTH = ARENA_WIDTH
    image_preprocessor.ARENA_HEIGHT = ARENA_HEIGHT
    ...
    # Initialize frame buffer before starting (capture initial state)
    # We need to render first to get a valid frame
    screen.fill(BG_COLOR)
    screen.blit(background_img, (ARENA_X_OFFSET, ARENA_Y_OFFSET))
    player.draw(screen)
    pygame.display.flip()
    
    # Add initial frames to buffer (repeat first frame 4 times)
    for _ in range(4):
        image_preprocessor.add_frame(screen)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_r:
                    ...
                    spawn_timer = 0
                    zombies_killed = 0
                    image_preprocessor.reset()
                    difficulty_manager.reset()
                    params = difficulty_manager.get_current_params()
                    # Re-initialize frame buffer after reset
                    for _ in range(4):
                        image_preprocessor.add_frame(screen)
        ...
        if player.health > 0:
            ...
            # DDA action (CNN-based)
            dda_action_timer += 1
            if use_dda and dda_action_timer >= dda_action_interval:
                dda_action_timer = 0
                
                # Capture current frame before making DDA decision (ensures we have latest state)
                image_preprocessor.add_frame(screen)
                
                # Get stacked frames for CNN
                stacked_frames = image_preprocessor.get_stacked_frames()  # (4, 96, 128, 3)
                cnn_state = stacked_frames.reshape(12, 96, 128)  # (12, 96, 128) for CNN
                
                # Get DDA action from CNN agent
                # Use stochastic sampling for more variety (not deterministic)
                # This will actually select actions based on their probabilities
                import time
                inference_start = time.time()
                action, log_prob, value = dda_agent.select_action(cnn_state, deterministic=False)
                inference_time = (time.time() - inference_start) * 1000  # Convert to ms
                
                # Debug: Get action probabilities to see what agent thinks
                import torch
                with torch.no_grad():
                    state_tensor = torch.from_numpy(cnn_state).float()
                    if state_tensor.dim() == 3:
                        state_tensor = state_tensor.unsqueeze(0)
                    state_tensor = state_tensor.to(dda_agent.device)
                    action_logits, _ = dda_agent.policy.forward(state_tensor)
                    ...
                    print(f"Action probabilities: {dict(zip(action_names, probs))} | Selected: {action_names[action]} | CNN Inference: {inference_time:.2f}ms")
                ...
                print(f"CNN DDA Action: {last_dda_action} | Zombie Speed: {params['ZOMBIE_SPEED']:.2f} | Spawn Rate: {params['SPAWN_RATE']:.0f}")
        ...
        # Only capture frame for CNN DDA when we're close to making a decision
        # This improves FPS - capture every 60 frames (1 second) instead of every frame
        # since DDA only acts every 300 frames (5 seconds at 60 FPS)
        # This ensures we have recent frames in the buffer without impacting FPS too much
        if use_dda and player.health > 0 and dda_action_timer % 60 == 0:
            image_preprocessor.add_frame(screen)
        ...
        if use_dda:
            ...
            # Title
            screen.blit(font.render("CNN DDA", True, YELLOW), (x_offset, y_start))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Play game with CNN DDA (Manual Mode)")
    parser.add_argument('--model', type=str, default='checkpoints_cnn/dda_cnn/best_model.pth',
                       help='Path to CNN DDA model')
    args = parser.parse_args()
    
    print("\n" + "="*50)
    print("ZOMBIE SHOOTER - CNN DDA MODE (YOU PLAY)")
    print("="*50)
    print("Controls:")
    ...
    print("  R - Restart")
    print("  ESC - Quit")
    print("\nThe CNN DDA agent adjusts difficulty every 5 seconds!")
