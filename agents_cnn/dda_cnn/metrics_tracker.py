"""
Training Metrics Tracker for CNN-based DDA Agent

Tracks comprehensive metrics during CNN DDA training and saves to JSON files.
Uses the same format as the existing feature-based DDA agent metrics tracker.
"""
...
class CNNDDAMetricsTracker:
    """
    Comprehensive metrics tracker for CNN-based DDA Agent training.
    
    Tracks and saves metrics after each episode and at training completion.
    Uses the same JSON format as the existing feature-based DDA agent metrics.
    """
    
    def __init__(self, save_dir: str = "checkpoints_cnn/dda_cnn/metrics"):
        """
    ...
    def end_episode(self, timestep: int, update_metrics: Optional[Dict] = None, 
        ...
        # Prepare metrics dictionary (same format as existing DDA tracker)
        episode_metrics = {
            'timestep': timestep,
    ...
    def finalize(self, total_timesteps: int):
        ...
        """
        # Create summary statistics (same format as existing DDA tracker)
        summary = {
        ...
        print(f"\nCNN DDA Metrics saved to: {self.metrics_file}")
