"""
Training Metrics Tracker for CNN-based Player Agent

Tracks comprehensive metrics during CNN training and saves to JSON files.
Uses the same format as the existing feature-based agent metrics tracker.
"""
...
class CNNPlayerMetricsTracker:
    """
    Comprehensive metrics tracker for CNN-based Player Agent training.
    
    Tracks and saves metrics after each episode and at training completion.
    Uses the same JSON format as the existing feature-based agent metrics.
    """
    
    def __init__(self, save_dir: str = "checkpoints_cnn/player_cnn/metrics"):
        """
    ...
    def end_episode(self, timestep: int, update_metrics: Optional[Dict] = None, 
        ...
        # Prepare metrics dictionary (same format as existing feature-based agent)
        episode_metrics = {
            'timestep': timestep,
    ...
    def finalize(self, total_timesteps: int):
        ...
        """
        # Create summary statistics (same format as existing tracker)
        summary = {
        ...
        print(f"\nCNN Metrics saved to: {self.metrics_file}")
